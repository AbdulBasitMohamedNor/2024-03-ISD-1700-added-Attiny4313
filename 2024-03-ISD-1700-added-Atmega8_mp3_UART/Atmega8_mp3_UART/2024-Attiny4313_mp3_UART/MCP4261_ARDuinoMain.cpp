...
// #define SPI_CLOCK            16 //arduino   <->   SPI Slave Clock Input     -> SCK (Pin 02 on MCP4261 DIP)
// #define SPI_MOSI             14 //arduino   <->   SPI Master Out Slave In   -> SDI (Pin 03 on MCP4261 DIP)
// #define SPI_MISO             15 //arduino   <->   SPI Master In Slave Out   -> SDO (Pin 13 on MCP4261 DIP)

// Then choose any other free pin as the Slave Select (pin 10 if the default but doesnt have to be)
#define MCP4261_SLAVE_SELECT_PIN 9 //arduino   <->   Chip Select               -> CS  (Pin 01 on MCP4261 DIP)
...
void setup()
{
	...
	Mcp4261.wiper0(5); // set pot0 rAW = 40% of max value
