...
static inline void setup()
{
	...
	DDRB &=~(1 << PINB1)|(1 << PINB5)|(1 << PINB6)|(1 << PINB7);//input
	PORTB |=(1 << PINB1)|(1 << PINB5)|(1 << PINB6)|(1 << PINB7);
...
{              /* Run every time button state changes */ // testa ISR(INT0_vect)
	...
	if (bit_is_clear(PINB, PINB1)&& loud == 1)
	{
		mcp4013_init_low();
		_delay_ms(200);
		loud = 0;
	}
	if (bit_is_clear(PINB,PINB1) && loud == 0) //
	{
		mcp4013_init_high();
		_delay_ms(200);
		loud =1;
