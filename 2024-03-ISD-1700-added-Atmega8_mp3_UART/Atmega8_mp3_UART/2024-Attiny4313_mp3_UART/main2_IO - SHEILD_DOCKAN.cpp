...
static inline void setup()
{
	...
	DDRB |= (1 << PINB4)|(1 << PINB2)| (1 << PINB3) ; //LED pwm output
	PORTB &= ~ (1 << PINB4)|(1 << PINB2)| (1 << PINB3);	 //off
	
	DDRB |= (1 << PINB7); //Transistor output
	PORTB &= ~(1 << PINB7);	 //off

	//PB1_BUSY pin as input
	DDRA &=~(1 << PINA1);//input
	PORTA  |=(1 << PINA1);
...
{    //TpowerOff();          /* Run every time button state changes */ // testa ISR(INT0_vect)
	...
	//::::::::::::::::::::::::::::::::::::::::::::::::
	if (bit_is_clear(PINB, PINB6))	   //läges väljaren knapp1
	{
		B1=0;		 TpowerON();
	...
	//::::::::::::::::::::::::::::::::::::::::::::::::
	/**/
	if (bit_is_clear(PINB, PINB5)) // bläddraren knapp 2
	{
		B1++;  		    TpowerON();
		//**********Läge1************************
		if (B1 == 1 && lage == 1)
...
int main(void)
{
	...
	while (1)
	{
		...
		count++;
		 PORTB ^=(1<<PINB3); //PORTB ^=(1<<PINB2);	 PORTB ^=(1<<PINB3);
		if(count==TotalTime)
		{		   
		count=0;
		stop();							  
		...
		_delay_ms(5);							  
		//POWER SAVE  
		TpowerOff(); //turn Power To DiyPlayer OFF 
		sleep_enable();
		sei();
		sleep_cpu();
		sleep_disable(); ////////
