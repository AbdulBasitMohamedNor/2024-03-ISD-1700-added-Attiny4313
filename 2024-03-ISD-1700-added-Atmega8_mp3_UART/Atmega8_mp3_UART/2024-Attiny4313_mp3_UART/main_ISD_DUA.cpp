...
static inline void setup_Mp3PlayerModule()
{
	toggle = 0;
	count = 0;
	TotalTime=30;
...
void initPinChangeInterrupt4(void)
{
	...
	PCMSK |= (1 << PCINT11) | (1 << PCINT12) | (1 << PCINT13) | (1 << PCINT14)| (1 << PCINT15)| (1 << PCINT16); /* set mask to look for PCINT13-15 */ // Behöver bara ha en Pin som interrupt
	//PCMSK1 |= (1 << PCINT10);
...
ISR(PCINT_B_vect)
{    TpowerOff();          /* Run every time button state changes */ // testa ISR(INT0_vect)
	count=0;
	//ledpwm_OFF();
	//setup();
	//initTimer1();
	...
	//::::::::::::::::::::::::::::::::::::::::::::::::
	if (bit_is_clear(PINB, PINB7))	   //läges väljaren knapp1
	{
		B1=0;
		...
		if (lage == 1)
		{	_delay_ms(300); //blink();
			play_10; stop();  // Natt bön Group sound run 15min. + Nashid
		}
		if (lage == 2)
		{	 _delay_ms(300);
			play_12; stop();	 // Morgon bön
		}
		if (lage == 3)
		{	_delay_ms(300);
			play_13; stop();	  //  sjunga
	...
	//::::::::::::::::::::::::::::::::::::::::::::::::
	/**/
	if (bit_is_clear(PINB, PINB6)) // bläddraren knapp 2
	{
		B1++;  
		//**********Läge1************************
		if (B1 == 1 && lage == 1)
		{	_delay_ms(200); play_15; }//stop(); // play_10=läge1 
			
											 
		if (B1 == 2 && lage == 1)
		{	
			_delay_ms(200); play_17; }//stop();

		if (B1 == 3 && lage == 1)
		{	
			_delay_ms(200); play_18; }//stop();

		if (B1 == 4 && lage == 1)
		{	
			_delay_ms(200);  play_19; B1=0;}//stop(); 
		//**********Läge2************************
		if (B1 == 1 && lage == 2)
		...
		if (B1 == 3 && lage == 4)
		{_delay_ms(200); play_19; stop();}

		if (B1 == 4 && lage == 4)
		{ _delay_ms(200); play_16; stop();B1=0;}
	}
	//::::::::::::::::::::::::::::::::::::::::::::::::
	if (bit_is_clear(PINB, PINB5))	//  Led Pin
	{ 	  	led++;
		...
		{	// led--;	led--;
			if (loud == 1 && bit_is_clear(PINB, PINB5))
			{
				blink();
				mcp4013_init_low();
				_delay_ms(500);
			...
			if (loud == 0 && bit_is_clear(PINB, PINB5))
			{	 blink();
				mcp4013_init_high();
				_delay_ms(500);
				loud =1;	
...
int main(void)
{
	...
	while (1)
	{
		...
		count++;
		 PORTB ^=(1<<PINB4); //PORTB ^=(1<<PINB2);	 PORTB ^=(1<<PINB3);
		if(count==TotalTime)
		{		   
		count=0;
		stop();
